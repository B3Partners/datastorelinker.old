	...
	$.widget("ui.formwizard", {
		...
		_init: function() {
			...
			if(!this.options.disableUIStyles){
				...
				this.element.addClass("ui-helper-reset ui-widget ui-helper-reset");
				this.element.find(":input").addClass("ui-helper-reset ui-state-default");
				//this.steps.addClass("ui-helper-reset ui-corner-all");
				this.steps.addClass("ui-helper-reset");
		...
		_next : function(){
            ...
            log(this.options);
            log("this.options.checkInputPresentSelector: " + this.options.checkInputPresentSelector);
            if (this.options.checkInputPresentSelector) {
                var inputParent = $("#" + this.currentStep).find(this.options.checkInputPresentSelector);
                log(inputParent);
                if (inputParent.length > 0 && inputParent.find("input").length == 0) {
                    log("no input elems");
                    return false;
                }
            }
		...
		_enableNavigation : function(){
            ...
            // jQuery UI bug: ui-state-hover class stays after we are no longer hovering. We remove it here.
			if($.trim(this.currentStep) !== this.steps.eq(0).attr("id")){
				this.backButton.removeAttr("disabled");
                
                if(!this.options.disableUIStyles){
					this.backButton.removeClass("ui-state-disabled ui-state-hover ui-state-focus").addClass("ui-state-default");
				}
			}

			this.nextButton.removeAttr("disabled");
			if(!this.options.disableUIStyles){
				this.nextButton.removeClass("ui-state-disabled ui-state-hover ui-state-focus").addClass("ui-state-default");
			}
		...
		_animate : function(oldStep, newStep, stepShownCallback){
			...
			old.animate(wizard.options.outAnimation, wizard.options.outDuration, wizard.options.easing, function(){
				current.animate(wizard.options.inAnimation, wizard.options.inDuration, wizard.options.easing, function(){
                    ...
                    //log("voor step shown callback aanroep");
		...
		_show : function(step){
			...
			if(this.currentStep !== step || step === this.firstStep){
				...
				var stepShownCallback = function() {
                    //if (triggerStepShown)
                        $(this.element).trigger('step_shown', $.extend({
                            "isBackNavigation" : backwards
                        }, this._state()));
                };
	  ...
	  /*Methods*/
		...
		destroy: function() {
			...
			this.nextButton.unbind("click").val(this.nextButtonInitinalValue).removeClass("ui-state-disabled").addClass("ui-state-default");
			this.backButton.unbind("click").val(this.backButtonInitinalValue).removeClass("ui-state-disabled").addClass("ui-state-default");
		...
		options: {
			...
			checkInputPresentSelector: ".mandatory-form-input" // Descendants of this selector will be checked for presence of input elements. If not present, the form (if any) will not validate. If this selector(-class) is not present as a descendant of a step, this validation step is ignored.
		}
    });
})(jQuery);
